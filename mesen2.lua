...
local function read_range(address, length)
    result = {}
    for i = 1,length do
        result[i] = emu.read(address + i - 1, emu.memType.nesDebug)
    end
...
local function register_write(address, callback)
    ...
    --    callback(value)
    --end
    --emu.addMemoryCallback(f, emu.callbackType.write, address)
	emu.addMemoryCallback(callback, emu.callbackType.write, address)
end
local function register_read(address, callback)
    emu.addMemoryCallback(callback, emu.callbackType.read, address)
end
local function register_exec(address, callback)
    emu.addMemoryCallback(callback, emu.callbackType.exec, address)
end
